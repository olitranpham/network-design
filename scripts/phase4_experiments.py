...
def write_gnuplot_script(results_dir: Path, avg_csv: Path, plot_png: Path) -> Path:
    gp_path = results_dir / "phase4_chart1.gp"
    gp = f"""\
set terminal pngcairo size 1400,800
set output "{plot_png.as_posix()}"
set title "Phase 4: Completion Time vs Loss/Error Rate"
...
def main():
    ...
    ap.add_argument("--runs", type=int, default=5, help="Successful OK per rate (>=5 required)")
    ap.add_argument("--max-attempts", type=int, default=30, help="Max attempts per (option,rate)")
    ap.add_argument("--window", type=int, default=4, help="GBN sender window size")
    ap.add_argument("--timeout", type=float, default=0.5, help="Sender retransmission timeout seconds")
    ap.add_argument("--hard-timeout", type=float, default=60.0, help="Hard cap per attempt")
    ...
    raw_csv = results_dir / "phase4_chart1_raw.csv"
    avg_csv = results_dir / "phase4_chart1_avg.csv"
    plot_png = results_dir / "phase4_chart1.png"
    ...
    for opt in option_names:
        for r_i, rate in enumerate(RATES):
            ...
            if ok_count < args.runs:
                print(
                    f"WARNING: {opt} rate={rate}% only got {ok_count}/{args.runs} OK runs after {attempt} attempts.",
                    flush=True,
                )
