...
def _kill_proc(p: Optional[subprocess.Popen]) -> None:
    """(prevents leftover sender/receiver processes)."""
    if p is None:
        return
    try:
        if p.poll() is None:
...
def run_one_transfer(
    ...
    """
    Start receiver, then run sender, then wait for receiver to exit.
    Returns: (seconds or None, status string)

    Status values:
      OK
      TIMEOUT (sender hung / did not exit in hard_timeout)
      SENDER_FAIL (sender exited non-zero)
      RECEIVER_TIMEOUT (receiver didn't exit in hard_timeout after sender finished)
      RECEIVER_FAIL (receiver exited non-zero)

    Important for rubric:
    - Sender/receiver stdout/stderr are suppressed (logging disabled during experiments).
    """
    assert option in ("opt1", "opt2", "opt3")

    p = rate_percent / 100.0

    # Keep ports/seeds unique even when retries happen
    ...
    recv_cmd = [
        ...
        "--out", out_path,
        "--seed", str(seed),
        "--linger", "0.2",   # small linger prevents "final ACK got corrupted" stalls
        "--quiet",           # keeps receiver from printing
    ]
    if option == "opt3":
        # option 3: DATA bit-errors at receiver-side receive path
    ...
    send_cmd = [
        ...
        "--seed", str(seed),
        "--quiet",            # keeps sender from printing
    ]
    if option == "opt2":
        # option 2: ACK bit-errors at sender-side receive path
...
def write_gnuplot_script(results_dir: Path, avg_csv: Path, plot_png: Path) -> Path:
    ...
    Plot uses columns 2/4/6 for averages.
    """
    gp_path = results_dir / "phase2d_plot.gp"
    gp = f"""\
set terminal pngcairo size 1200,700
set output "{plot_png.as_posix()}"
set title "Phase 2(d): Completion Time vs Loss/Error Rate"
...
plot "{avg_csv.as_posix()}" using 1:2 with linespoints title "Option 1 (No errors)", \
...
def main():
    ...
    ap.add_argument("--timeout", type=float, default=0.05, help="Sender ACK wait timeout seconds (smaller = faster retries)")
    ap.add_argument("--hard-timeout", type=float, default=15.0, help="Hard cap per attempt (kills hung sender/receiver)")
    ...
    ap.add_argument("--recv-bind-wait", type=float, default=0.05, help="Seconds to wait after starting receiver before sender starts")
    ap.add_argument("--plot", action="store_true", help="Generate PNG plot using gnuplot if available")
    args = ap.parse_args()

    # at least 5 runs per rate
    if args.runs < 5:
    ...
    raw_csv = results_dir / "phase2d_raw.csv"
    avg_csv = results_dir / "phase2d_avg.csv"
    plot_png = results_dir / "phase2d_plot.png"

    raw_rows: List[dict] = []

    times_ok: Dict[str, List[List[float]]] = {
        "opt1": [[] for _ in range(len(RATES))],
        "opt2": [[] for _ in range(len(RATES))],
        "opt3": [[] for _ in range(len(RATES))],
    }
    ...
    for opt in ("opt1", "opt2", "opt3"):
        for r_i, rate in enumerate(RATES):
            ...
            while ok_count < args.runs and attempt < args.max_attempts:
                out_path = str(results_dir / f"tmp_{opt}_{rate}_ok{ok_count}_att{attempt}.bin")
                ...
                raw_rows.append(
                    {
                        "option": opt,
                        "rate_percent": rate,
                        "run": ok_count,      # OK-run slot we were trying to fill
    ...
    write_csv(
        avg_csv,
        avg_rows,
        fieldnames=["rate_percent", "opt1_avg", "opt1_ok_runs", "opt2_avg", "opt2_ok_runs", "opt3_avg", "opt3_ok_runs"],
    )
